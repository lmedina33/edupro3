...
class _pdf
{
	...
	function multitable($table, $left, $top, $padding, $fontsize = 10, $border = 0, $args = false)
	{
		foreach ($table as $i => $tr)
		{
			if ($tr === false)
		...
		$rest_width = $viewport - $all_width - ($left);
		$rest_no_width = $rest_width / count($no_widths);
		
		foreach ($no_widths as $i)
		{
			$widths[$i] = $rest_no_width;
		}
		ksort($widths);
		
		if ($args !== false && isset($args['last_height']))
		{
			$all_height += $args['last_height'];
		}
		...
		foreach ($table as $i => $tr)
		{
			...
			if ($border)
			{
				...
				{
					// Left
					$this->cp->line($border['left'], $this->cp->cy($pos_top - $fontsize - 2), $border['left'], $this->cp->cy($max_top));
					
					// Right
					if (isset($border['right']))
					{
						$this->cp->line($border['right'], $this->cp->cy($pos_top - $fontsize - 2), $border['right'], $this->cp->cy($max_top));
					}
				}
			}
		}
